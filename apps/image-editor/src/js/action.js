...
export default {
  ...
  setReAction() {
    this.on({
      ...
      addText: (pos) => {
        const { textColor: fill, textBgColor: textBackgroundColor, fontSize, fontStyle, fontWeight, underline } = this.ui.text;
        const fontFamily = 'Noto Sans';

        this.addText('Double Click', {
          position: pos.originPosition,
          styles: { fill, textBackgroundColor, fontSize, fontFamily, fontStyle, fontWeight, underline },
        }).then(() => {
          this.changeCursor('default');
        });
      },
